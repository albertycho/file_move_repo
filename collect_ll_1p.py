...
f_ll1p_stats=open('ll_1P_stats_collected.csv','w')
f_ll1p_stats.write('app,ddr_ipc, cxl_ipc,ddr_mbw,cxl_mbw,ddr_mlat,cxl_mlat,\n')
for i in range(len(appNames)):
   f_ll1p_stats.write(appNames[i]+','+str(DDR_ipcs[i])+','+str(CXL_ipcs[i])+','+str(DDR_mbws[i])+','+str(CXL_mbws[i])+','+str(DDR_mlats[i])+','+str(CXL_mlats[i])+',\n' )
